...
model = ast2vec1.load_model()

# and encode all the syntax trees as points
X = ast2vec1.encode_trees(model, trees)
...
ast2vec1.interpolation_plot(model, start_tree = 0, target_tree = tree_index, X = X, variable_classifier = vc)
plt.show()

# construct the dynamical system via linear regression
W = ast2vec1.construct_dynamical_system(tree_index, X, tree_traces)
# visualize the resulting dynamical system
plt.figure(figsize = (15, 10))
ast2vec1.dynamical_system_plot(W, start_tree = 0, target_tree = tree_index, X = X, arrow_scale = 1.5, model = model, variable_classifier = vc)
plt.show()
